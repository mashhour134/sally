...
let systemLogs: SystemLog[] = [
  {
    ...
    level: "info",
    message: "أقلاع ناجح لنظام Air Guard وبدء مزامنة القنوات وتلقي بيانات ESP32.",
    source: "System"
  },
  {
    id: "log-2",
  ...
  if (dev.aqi > 200) {
    ...
    if (!hasDanger) {
      alerts.unshift({
        ...
        severity: "danger",
        value: dev.sensorValue,
        message: `تنبيه أحمر خارجي: مستوى جودة هواء خطير تم كشفه بـ ${dev.name}. القيمة: ${dev.sensorValue} PPM.`,
        timestamp: new Date().toISOString(),
        resolved: false
      });
...
3.  **إرشادات الصيانة للجهاز (ESP32):** تأكد من نظافة الشبكة الخارجية لمستشعر الغاز وخلوها من الرطوبة والشوارد التي تؤثر على توازن الشمعة البلاتينية للمستشعر MQ135.

---
*هذا التقرير تم تجميعه تلقائياً بواسطة وحدة الذكاء الاصطناعي المساندة لـ **Air Guard**.*`;
    return res.json({ report: fallbackReport, isMockDemo: true });
  }

  try {
    const prompt = `أنت خبير كيميائي واستشاري بيئي تقود نظام الرعاية الصحية الذكي "Air Guard".
...
5. خاتمة وتوقيع إداري لنظام Air Guard.
